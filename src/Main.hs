...
foundMessage (Person subject _ _) (Person object channel want) = 
    SimpleMessage (EmojiIcon "rainbow") botName channel ("@" <> u subject <> " " <> body)
  where 
    body = 
...
wantMessage (Person user channel want) = 
    SimpleMessage (EmojiIcon "hand") botName channel ("@" <> u user <> " " <> body)
  where
    body = 
        case want of 
            WantsUser objectDesire ->
...
parseCommand (Command user _ maybeText) = 
    case (cheatMode, (map TL.strip . TL.splitOn "--") <$> maybeText) of
        (True, Just [text', user']) -> (User user', parseWant (return text'))
        _ -> (user, parseWant maybeText)
...
hi5 dbM config (Just command@(Command _ channel _)) = do
  ...
  where
    ...
    person = Person user channel want
hi5 _ _ Nothing = do
    return $ "My name is " <> botName <> ". Hello everybody."
...
main = do
    ...
    run port (slashSimple (hi5 db (Config "bukalapak.slack.com" token)))
  where
    port = 8000